...
- 실제 `workflow_dispatch` 실행에서 App 인증/저장소 생성/contents API 읽기는 성공했지만, `/actions/variables` API는 `Resource not accessible by integration`이 재현되었습니다.
- 현재 프로비저닝/정책 경로는 이 한계를 회피하도록 `REQUESTER_LOGIN` 변수 API에 의존하지 않고, `.github/provisioning/requester-metadata.json` 파일을 canonical source of truth로 사용합니다.
...
- requester metadata contract using `.github/provisioning/requester-metadata.json` as canonical source of truth
